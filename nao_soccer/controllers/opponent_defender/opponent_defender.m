    ...
    case 'track_mode_top'
      ...
      if (distance_top > distanceThreshold)
        ...
        elseif (distance < -yDistThreshold)
          ...
          % 检查是否需要进入 approach 状态
          if (distance_top < 1)
            state = 'approach';  % 切换到靠近模式
            continue;  % 立即切换状态，跳过后续逻辑
          end
        end
